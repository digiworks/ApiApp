class ReactValidator {
    ...
    constructor(options = {}) {
        this.fields = {}; /* array of fields status if evaluated*/
        this.visibleFields = [];
        this.errorMessages = {}; /* array of error messages*/
        this.messagesShown = false;
        this.fieldsRules = {}; /* array of fields rules */
        this.rules = {
            ...
            same:{message: 'The :attribute field must be equal to :field', rule: (val,params, validator) => { let element = document.querySelector("#"+params[0]); if(element){ return val == element.value}else{ return false}}, required: true, messageReplace: (message, params) => message.replace(':field', params[0])},
        ...
        // apply language
        if (options.locale && !ReactValidator.locales.hasOwnProperty(options.locale)) {
            console.warn('Locale not found! Make sure it is spelled correctly and the locale file is loaded.');
        }
        const locale = ReactValidator.locales[options.locale] || {};
    ...
    messageAlways(field, message, options = {}) {
        console.warn('The messageAlways() method is deprecated in ReactValidator. Please see the documentation and switch to the messageWhenPresent() method.')
    ...
    addFieldRules(field, rules) {
        this.fieldsRules[field] = rules;
    }

    getFieldErrorMessages(field) {
        return  this.errorMessages[field] ? this.errorMessages[field] : "";
    }

    /**
     * 
     */
    checkField(field, inputValue, options = {}){
        let message = "";
        let rules = this.fieldsRules[field];
        if (rules) {
            let ret = this.checkValidations(field, inputValue, rules, options);
            if (!ret.result) {
                message = ret.msg;
            }
        }
     ...
     * 
     */
    checkValidations(field, inputValue, validations, options = {}) {
        this.errorMessages[field] = null;
        this.fields[field] = true;
        if (!Array.isArray(validations)) {
            validations = validations.split('|');
